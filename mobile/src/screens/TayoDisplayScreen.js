    ...
    <View style={[styles.container, { backgroundColor: theme.background }, { backgroundColor: theme.background }]}>
      <View style={[styles.headerRow, { justifyContent: 'flex-end' }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          {(role === 'admin' || role === 'co-principal' || role === 'principal' || role === 'assistant-principal') && (
...
const getStyles = (theme, isDarkMode) => StyleSheet.create({
  ...
  card: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: theme.cardBackground, marginHorizontal: 15, marginTop: 12, padding: 15, borderRadius: 16, borderWidth: 1, borderColor: theme.borderColor, ...Platform.select({ ios: { shadowColor: '#2f4360', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 8 }, android: { elevation: 3 } }) },
  ...
  modalContent: { backgroundColor: theme.cardBackground, padding: 25, borderTopLeftRadius: 25, borderTopRightRadius: 25, minHeight: '60%', ...Platform.select({ ios: { shadowColor: '#000', shadowOffset: { width: 0, height: -4 }, shadowOpacity: 0.1, shadowRadius: 10 }, android: { elevation: 10 } }) },
